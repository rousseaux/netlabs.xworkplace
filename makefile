...
!ifdef XWP_DEBUG_CLASSES    #changed V1.0.9 (2011-10-21) [rwalsh]: added XWP_DEBUG_CLASSES
...
# changed V1.0.9 (2011-10-21) [rwalsh]: added 'idl' as a dependency for 'dep'
# If the .h and .ih files emitted by the SOM compiler haven't been created
# yet, fastdep.exe will not include them in its '.depend' files.  Without
# them, some source files may not be rebuilt when an IDL file changes.
# @@changed V1.0.9 (2012-02-21) [slevine]: pass PROJECT_BASE_DIR rather than
# SUBMAKE_PASS_STRING to avoid nmake long path crashes
dep: idl
    ...
    @$(MAKE) -nologo dep "NOINCLUDEDEPEND=1" "PROJECT_BASE_DIR=$(PROJECT_BASE_DIR)"
    @cd $(CURRENT_DIR)
    @echo ----- Leaving $(MAKEDIR)
    @echo Yo, done!
...
!else
    ...
    $(COPY) release\install\xwp_o.ico $(XWPRUNNING)\install
!endif
...
# SUBMAKE_PASS_STRING to avoid nmake long path crashes
    @echo $(MAKEDIR)\makefile [$@]: Going for src\helpers (DLL version)
    @cd $(HELPERS_BASE)\src\helpers
    $(MAKE) -nologo all "MAINMAKERUNNING=YES" "PROJECT_BASE_DIR=$(PROJECT_BASE_DIR)" \
...
#      $(XWPRUNNING)\plugins\xcenter\xwHealth.dll \     disabled V1.0.1 (2003-02-02) [umoeller]
...
$(MODDEFFILE): include\bldlevel.h
        @$(RUN_BLDLEVEL) $@ $? "$(XWPNAME) main module"
...
src\shared\xwpres.def: include\bldlevel.h
        @$(RUN_BLDLEVEL) $@ $? "$(XWPNAME) resources module"
...
src\widgets\winlist.def: include\bldlevel.h
        @$(RUN_BLDLEVEL) $@ $? "$(XWPNAME) window-list plugin DLL"
...
src\widgets\w_diskfree.def: include\bldlevel.h
        @$(RUN_BLDLEVEL) $@ $? "$(XWPNAME) diskfree plugin DLL"
...
src\widgets\ipmon.def: include\bldlevel.h
        @$(RUN_BLDLEVEL) $@ $? "$(XWPNAME) IP monitor plugin DLL"
...
src\widgets\monitors.def: include\bldlevel.h
        @$(RUN_BLDLEVEL) $@ $? "$(XWPNAME) monitors plugin DLL"
...
src\widgets\sentinel.def: include\bldlevel.h
        @$(RUN_BLDLEVEL) $@ $? "$(XWPNAME) memory sentinel plugin DLL"
...
src\widgets\xwHealth.def: include\bldlevel.h
        @$(RUN_BLDLEVEL) $@ $? "$(XWPNAME) xwHealth plugin DLL"
...
src\widgets\sample.def: include\bldlevel.h
        @$(RUN_BLDLEVEL) $@ $? "$(XWPNAME) sample plugin DLL"
...
src\widgets\d_cdfs.def: include\bldlevel.h
        @$(RUN_BLDLEVEL) $@ $? "$(XWPNAME) CDFS.IFS driver plugin DLL"
...
$(MODULESDIR)\xwpfonts.fon: $(XWP_OUTPUT_ROOT)\$(@B).res src\shared\$(@B).def
...
src\shared\xwpfonts.def: include\bldlevel.h
        @$(RUN_BLDLEVEL) $@ $? "$(XWPNAME) bitmap fonts"
