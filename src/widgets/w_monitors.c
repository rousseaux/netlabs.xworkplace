...
// headers in /helpers
#include "helpers\apmh.h"               // Legacy Advanced Power Management helpers
...
PACPIHGETPOWERSTATUS pacpihGetPowerStatus = NULL;
PACPIHOPEN pacpihOpen = NULL;

// @@changed V1.0.9 (2012-02-21) [slevine]: import more ACPI helpers
static const RESOLVEFUNCTION G_aImports[] =
    {
        ...
        "acpihOpen", (PFN*)&pacpihOpen,
        "acpihGetPowerStatus", (PFN*)&pacpihGetPowerStatus
    };
                   ...
                   PXCENTERWIDGET pWidget)
{
    ...
    switch (pPrivate->ulType)
    {
        ...
        */
        ...
        case MWGT_POWER:
        {
            // @@changed V1.0.9 (2012-02-21) [slevine]: rework to support
            // ACPI power status reporting. Code by David Azarewicz.
            ACPI_API_HANDLE hACPI;

            pWidget->ulHelpPanelID = ID_XSH_WIDGET_POWER_MAIN;
            ulUpdateFreq = 10 * 1000;       // once per minute

            /* First try ACPI for power status
             * The power statistics should really be in a generic structure, not the
             * legacy APM structure, but for now I will fill in the legacy APM
             * structure so I don't have to change much of the code.
             */
            if (!(pPrivate->arcAPM = pacpihOpen(&hACPI)))
            {
                if (pPrivate->pApm == NULL)
                {
                    // we have to allocate the APM structure here because we may never
                    // call apmhOpen.
                    if (!(pPrivate->pApm = NEW(APM)))
                        pPrivate->arcAPM = 1;
                    else
                        ZERO(pPrivate->pApm);
                }
                ...
                if (pPrivate->pApm)
                    pPrivate->arcAPM = pacpihGetPowerStatus(pPrivate->pApm, NULL);
            }

            /* If for some reason we got an error getting the power status from ACPI,
             * arcAPM will be non-zero so we see if the legacy APM interface is available.
             */
            if (pPrivate->arcAPM)
            {
                if (!(pPrivate->arcAPM = papmhOpen(&pPrivate->pApm)))
                {
                    ...
                    else
                    {
                        ...
                        pPrivate->arcAPM = papmhReadStatus(pPrivate->pApm, NULL);
                    }
                }
            }
                ...
                if (usNotifyCode == TTN_NEEDTEXT)
                {
                    ...
                    switch (pPrivate->ulType)
                    {
                        ...
                        case MWGT_TIME:
                        {
                            ...
                            pnlsDateTime((pPrivate->ulType == MWGT_TIME) ? pPrivate->szDateTime : NULL,
                ...
                BOOL fDrawFrame)
{
    ...
    switch (pPrivate->ulType)
    {
        ...
        case MWGT_TIME:
        {
            ...
            pnlsDateTime((pPrivate->ulType == MWGT_DATE) ? szPaint : pPrivate->szTooltipText,  // V1.0.8 (2007-01-14) [pr]
       ...
       )
    {
        switch (usTimerID)
        {
                        ...
                        // except diskfree
            {
                ...
                // @@changed V1.0.9 (2012-02-21) [slevine]: rework to support
                // ACPI power status reporting.  Code by David Azarewicz.
                if ((pPrivate->ulType == MWGT_POWER) && pPrivate->pApm)
                {
                    BOOL fChanged = FALSE;
                    /* First try ACPI for power status
                     * The power statistics should really be in a generic structure,
                     * not the legacy APM structure, but for now I will fill in the
                     * legacy APM structure so I don't have to change much of the code.
                     */
                    pPrivate->arcAPM = pacpihGetPowerStatus(pPrivate->pApm, &fChanged);
                    if (pPrivate->arcAPM)
                        pPrivate->arcAPM = papmhReadStatus(pPrivate->pApm, &fChanged);
 ...
 *@@changed V1.0.5 (2005-11-26) [pr]: now posting msg to t1 obj for opening objects @@fixes 398
                         ...
                         MPARAM mp1)
{
       ...
       )
    {
        ...
        if (hobj)
        {
            // changed V1.0.5 (2005-11-26) [pr] @@fixes 398
