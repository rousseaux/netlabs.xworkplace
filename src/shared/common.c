        ...
        while (fRetry)      // V0.9.19 (2002-04-24) [umoeller]
        {
            ...
            if (hmodLoaded)
            {
                ...
                if (fEnforceReload)
                {
                    ...
                    // no enforce reload: that's OK always
                    hmodReturn = hmodLoaded;
            } // end if (hmodLoaded)
...
// #endif
...
#endif
#ifndef __NOTURBOFOLDERS__
        // sfExtAssocs, FIELDOFFSET(OLDGLOBALSETTINGS, __fExtAssocs), 4,        removed V1.0.1 (2002-12-15) [umoeller]
        //     SP_SETUP_FEATURES, 0,
        //     "fExtAssocs",
        ...
        sulSBStyle, FIELDOFFSET(OLDGLOBALSETTINGS, SBStyle), 4,
            SP_27STATUSBAR, SBSTYLE_WARP4MENU,  // Overridden in cmnLoadGlobalSettings()
            "ulSBStyle",
        slSBBgndColor, FIELDOFFSET(OLDGLOBALSETTINGS, lSBBgndColor), 4,
            SP_27STATUSBAR, RGBCOL_GRAY,        // Overridden in cmnLoadGlobalSettings()
            "lSBBgndColor",
        slSBTextColor, FIELDOFFSET(OLDGLOBALSETTINGS, lSBTextColor), 4,
            SP_27STATUSBAR, RGBCOL_BLACK,       // Overridden in cmnLoadGlobalSettings()
            "lSBTextColor",
 ...
 *@@changed V1.0.9 (2011-10-25) [rwalsh]: strip 0xFF from the Product Info string
 */
                        ...
                        ULONG ulSound)      // in: sound intex to play
{
    ...
    // Replace 0xff ("required space") with a standard space character
    // because TextView uses 0xff as a formatting escape character.
    // (Actually, it's worse than that - look at txvStripLinefeeds()).
    // changed V1.0.9 (2011-10-25) [rwalsh]
    pChar = strInfo.psz;
    while ((pChar= strchr(pChar, 0xFF)) != NULL)
        *pChar = 0x20;
