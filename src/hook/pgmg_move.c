 ...
 *      is started (StartPageMage in xwpdaemn.c).
...
VOID _Optlink fntMoveThread(PVOID pvDummy)
{
    ...
    while (!byQuit)
    {
        ...
        switch (byMsgType)
        {
            ...
            case PGMGQ_MOUSEMOVE:
                ...
                if (ulParm1 & MOVE_UP)
                    ...
                    lDeltaY = G_ptlMoveDelta.y;
                if (G_PageMageConfig.bRepositionMouse)
                {
            ...
            case PGMGQ_CLICK2ACTIVATE:
            case PGMGQ_CLICK2LOWER:
                // set lDeltas to which desktop we want
                lDeltaX = ulParm1 / (G_ptlEachDesktop.x + 1);
                lDeltaY = G_PageMageConfig.ptlMaxDesktops.y - 1 - ulParm2
            ...
            case PGMGQ_FOCUSCHANGE:
            {
                ...
                if (!(swpActive.fl & (SWP_HIDE | SWP_MINIMIZE)))
                {
                    ...
                    if (    (lAbsX >= 0)
                         && (lAbsX <= (G_PageMageConfig.ptlMaxDesktops.x
                                       * G_pHookData->lCXScreen))
                         && (lAbsY >= 0)
                         && (lAbsY <= (G_PageMageConfig.ptlMaxDesktops.y
        ...
        if (lDeltaX || lDeltaY)
        {
            ...
            if (bReturn)
            {
                ...
                {
                    WinSetWindowPos(G_pHookData->hwndPageMageFrame, HWND_TOP,
                                    0, 0, 0, 0, SWP_ZORDER | SWP_SHOW | SWP_RESTORE);
                    DosSleep(G_PageMageConfig.ulFlashDelay);
                    ...
                    BOOL bAllowUpdate)
{
    ...
    for (usIdx = 0; usIdx < usMoveListCtr; usIdx++)
    {
        ...
        swp[usIdx].hwndInsertBehind = HWND_TOP;
        WinSetWindowPos(swp[usIdx].hwnd, NULLHANDLE, swp[usIdx].x, swp[usIdx].y,
...
INT pgmmZMoveIt(LONG lXDelta, LONG lYDelta)
{
    ...
    lRightEdge = G_PageMageConfig.ptlMaxDesktops.x * G_pHookData->lCXScreen;
    lBottomEdge = G_PageMageConfig.ptlMaxDesktops.y * G_pHookData->lCYScreen;
    ...
    if (lXDelta != 0)
    {
        if (lNewPosX < 0)
        {
            if (G_ptlCurrPos.x == 0)
            {
                if (G_PageMageConfig.bWrapAround)
                    lXDelta = - (LONG)((G_PageMageConfig.ptlMaxDesktops.x - 1) * G_pHookData->lCXScreen);
                else
                    lXDelta = 0;
            }
            else
        ...
        else
        {
            if ((lNewPosX + G_pHookData->lCXScreen) >= lRightEdge)
            {
                ...
                {
                    if (G_PageMageConfig.bWrapAround)
                        lXDelta = (G_PageMageConfig.ptlMaxDesktops.x - 1) * G_pHookData->lCXScreen;
                    else
                        lXDelta = 0;
                }
                else
    ...
    if (lYDelta != 0)
    {
        if (lNewPosY < 0)
        {
            ...
            {
                if (G_PageMageConfig.bWrapAround)
                    lYDelta = (G_PageMageConfig.ptlMaxDesktops.y - 1) * G_pHookData->lCYScreen;
                else
                    lYDelta = 0;
            } else
        ...
        else
        {
            if ((lNewPosY + G_pHookData->lCYScreen) >= lBottomEdge)
            {
                ...
                {
                    if (G_PageMageConfig.bWrapAround)
                        lYDelta = - (LONG)((G_PageMageConfig.ptlMaxDesktops.y - 1) * G_pHookData->lCYScreen);
                    else
                        lYDelta = 0;
                } else
...
/* VOID _Optlink fntMouseMoveThread(PVOID pvDummy)
{
    ...
    while (TRUE) {
      if (!G_bLockup) {
        ...
        if (G_PageMageConfig.bPanAtLeft && (G_siVD.sLeftEdge >= G_PageMageConfig.iEdgeBoundary)) {
          G_siVD.sLeftEdge = 0;
          lOption |= MOVE_LEFT;
        }

        if (G_PageMageConfig.bPanAtRight && (G_siVD.sRightEdge >= G_PageMageConfig.iEdgeBoundary)) {
          G_siVD.sRightEdge = 0;
          lOption |= MOVE_RIGHT;
        }

        if (G_PageMageConfig.bPanAtBottom && (G_siVD.sBottomEdge >= G_PageMageConfig.iEdgeBoundary)) {
          G_siVD.sBottomEdge = 0;
          lOption |= MOVE_DOWN;
        }

        if (G_PageMageConfig.bPanAtTop && (G_siVD.sTopEdge >= G_PageMageConfig.iEdgeBoundary)) {
          G_siVD.sTopEdge = 0;
          lOption |= MOVE_UP;
        }
      ...
      DosSleep(G_PageMageConfig.ulSleepTime);
    } // while
