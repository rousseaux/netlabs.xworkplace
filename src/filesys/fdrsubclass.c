...
#include "shared\classtest.h"           // various somIsA() wrappers
 ...
 *@@changed V1.0.9 (2011-09-17) [rwalsh]: set source object for "Selected' menu
 */
                           ...
                           HWND hwndMenuMsg)          // in: mp2 from WM_INITMENU
{
    switch (sMenuID)
    {
        ...
        {
            PMPF_MENUS(("  'Selected' pulldown found"));
            psfv->pSourceObject = wpshQuerySourceObject(psfv->somSelf,
                                                        psfv->hwndCnr,
                                                        TRUE,      // selected mode
                                                        &psfv->ulSelection);
 ...
 *@@changed V1.0.9 (2011-09-11) [rwalsh]: corrected menuids/open-view values @@fixes 4
 */
        ...
        if (hwndCnr)
        {
            ...
            if (pObject = objResolveIfShadow(pObject))
            {
                if (WinGetKeyState(HWND_DESKTOP, VK_SHIFT) & 0x8000)
                {
                    ...
                    // V1.0.9 (2011-09-11) [rwalsh]: initial implementation
                    // confused OPEN_* values with WPMENUIDs - they aren't
                    // always the same @@fixes 4
                    ...
                    if (ulView)
                    {
                        ULONG  ulOldDefaultView;
                        WPObject *pTarget;

                        // for disks, set the default view for the root folder -
                        // the disk's defview will automatically be reset to match
                        // changed V1.0.9 (2012-02-15) [rwalsh]: @@fixes 1193
                        if (   !ctsIsDisk(pObject)
                            || !(pTarget = _wpQueryRootFolder(pObject)))
                            pTarget = pObject;
                        
                        ulOldDefaultView = _wpQueryDefaultView(pTarget);

                        // we make sure this fails for the desktop
                        // and the split view
                        // V1.0.0 (2002-09-13) [umoeller]
                        if (    (ulView != ulOldDefaultView)
                             && (_wpSetDefaultView(pTarget,
                                                   ulView))
                           )
                        {
                            USHORT  usOldDefaultItem = 
                                    (USHORT)WinSendMsg((HWND)mp2,
                                                       MM_QUERYDEFAULTITEMID,
                                                       0,
                                                       0);

                            PMPF_MENUS(("  setting default menuid 0x%lX in hMenu 0x%lX",
                                        usItem,
                                        mp2));

                            WinSendMsg((HWND)mp2,
                                       MM_SETDEFAULTITEMID,
                                       MPFROM2SHORT(usItem, 0),
                                       0);

                            winhSetMenuItemChecked((HWND)mp2,
                                                   MPFROM2SHORT(usOldDefaultItem, 0),
                                                   FALSE);

                            _wpSaveDeferred(pTarget);
                        }
                ...
                // now call the functions in fdrcommand.c for this,
