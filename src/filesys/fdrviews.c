 ...
 *
 *@@added V1.0.0 (2002-08-24) [umoeller]
 *@@changed V1.0.9 (2011-10-14) [rwalsh]: commented-out unused function
 */
#if 0
...
/*
 *@@ ScaleSrcToDst:
 *      This provides two types of proportional scaling:
 *
 *      -- downscale-only:  the result will never be larger than
 *            the smaller of the source or destination rectangles
 *
 *      -- upscale or downscale:  when the SCALEF_EXPAND flag is
 *            set, one dimension of the result will equal the
 *            destination's while the other will be greater than
 *            or equal to the destination's corresponding dimension 
 *
 *      If the SCALEF_CENTER flag is set, the result will be centered
 *      in the destination's rectangle.  If the SCALEF_INCL flag is
 *      set, the result will be converted to an inclusive rectangle.
 *
 *@@added V1.0.9 (2011-10-14) [rwalsh]: @@fixes 277
 */

#define SCALEF_NONE     0
#define SCALEF_CENTER   1
#define SCALEF_INCL     2
#define SCALEF_EXPAND   4
#define SCALEF_TILED    8

void  ScaleSrcToDst(PSIZEL pszlSrc,
                    PSIZEL pszlDst,
                    PRECTL prclRtn,
                    ULONG  ulFlags)
{
    if (ulFlags & SCALEF_EXPAND)
    {
        // Scale the source so that one dimension equals
        // the destination and the other is GTE to it.
        prclRtn->yTop = pszlDst->cy;
        prclRtn->xRight = (pszlSrc->cx * prclRtn->yTop) / pszlSrc->cy;

        if (prclRtn->xRight < pszlDst->cx)
        {            
            prclRtn->xRight = pszlDst->cx;
            prclRtn->yTop = (pszlSrc->cy * prclRtn->xRight) / pszlSrc->cx;
        }
    }
    else
    {
        if (pszlSrc->cy <= pszlDst->cy)
        {
            // If both source dimensions are smaller than
            // the destination, use them as-is.  Otherwise,
            // use the full width of the destination and
            // scale down the source's height.
            if (pszlSrc->cx <= pszlDst->cx)
            {
                prclRtn->yTop = pszlSrc->cy;
                prclRtn->xRight = pszlSrc->cx;
            }
            else
            {
                prclRtn->xRight = pszlDst->cx;
        ...
        {
            // Scale the source so that one dimension equals
            // the destination and the other is LTE to it.
            prclRtn->yTop = pszlDst->cy;
            prclRtn->xRight = (pszlSrc->cx * prclRtn->yTop) / pszlSrc->cy;
    ...
    PMPF_CNRBITMAPS(("src= %d.%d  dst= %d.%d  rtn= %d.%d  adj= %d.%d  flags= %s%s%s\n",
                    pszlSrc->cx, pszlSrc->cy,
                    pszlDst->cx, pszlDst->cy,
                    prclRtn->xRight, prclRtn->yTop,
                    pszlDst->cx - prclRtn->xRight, pszlDst->cy - prclRtn->yTop,
                    (ulFlags & SCALEF_CENTER) ? "C" : "",
                    (ulFlags & SCALEF_INCL)   ? "I" : "",
                    (ulFlags & SCALEF_EXPAND) ? "X" : ""));

    if (ulFlags & SCALEF_CENTER)
    {
        LONG  lAdj;

        // deal with the differences being an odd nbr of pixels
        lAdj = pszlDst->cy - prclRtn->yTop;
        prclRtn->yBottom = lAdj / 2;
        prclRtn->yTop += lAdj - prclRtn->yBottom;

        lAdj = pszlDst->cx - prclRtn->xRight;
        prclRtn->xLeft = lAdj / 2;
        prclRtn->xRight += lAdj - prclRtn->xLeft;
    }

    if (ulFlags & SCALEF_INCL)
    {
        prclRtn->yTop--;
        prclRtn->xRight--;
    }
...
/*
 *@@ BlitClippedBitmap:
 *      Sets a clipping rectangle then blits a bitmap.
 *      If the source and destination rectangles aren't
 *      equal, the output will be expanded or compressed.
 *
 *@@added V1.0.9 (2011-10-14) [rwalsh]: @@fixes 277
 */

void  BlitClippedBitmap(HPS hps,
                        HBITMAP hbm,
                        PPOINTL pptl,
                        PRECTL prclClip)
{
    ...
    // reset clip region to "all" to quickly get the
    // old clip region; we must save and restore that,
    // or the cnr will stop painting quickly
    GpiSetClipRegion(hps, NULLHANDLE, &hrgnOldClip);

    // adjust the rectl to be inclusive,
    // then set it as the new clip region
    rclAdj = *prclClip;
    rclAdj.xRight--;
    rclAdj.yTop--;
    GpiIntersectClipRectangle(hps, &rclAdj);
 ...
 *
 *@@added V1.0.0 (2002-08-24) [umoeller]
 *@@changed V1.0.9 (2011-10-14) [rwalsh]: rewritten to implement proportional scaling and tiling @@fixes 277
 */

MRESULT PaintCnrBackground(HWND hwndCnr,
                           POWNERBACKGROUND pob)
{
    PSUBCLCNR     pSubCnr;
    PIBMFDRBKGNDSTORE pBkgndStore;
    POINTL        aptl[4];
    SIZEL         szl,
                  szlAdj;
    LONG          lStart,
    ...
    // we need not paint the background if we have a bitmap
    // AND it should fill the entire folder (avoid flicker)
    if (    (!pSubCnr->hbm)
         || (BKGND_NORMAL == pBkgndStore->usTiledOrScaled)
       )
    {
        gpihSwitchToRGB(pob->hps);
        WinFillRect(pob->hps,
                    &pob->rclBackground,
                    pSubCnr->lcolBackground);
    }

    // if there's no bitmap or any of the dimensions are zero, exit
    if (    (!pSubCnr->hbm)
    ...
    switch (pBkgndStore->usTiledOrScaled)
    {
        /*
         * BKGND_NORMAL:
         *      scale the bitmap down if needed (but never up),
         *      and center it in the container
         */
        case BKGND_NORMAL:
            ulFlags = SCALEF_CENTER | SCALEF_INCL;
        break;

        /*
         * BKGND_SCALED:
         *      for 1x1, scale the bitmap to match the container
         *      size while maintaining its aspect ration.  One
         *      dimension will equal the container's while the
         *      other will be GTE to it.  The oversized dimension
         *      will be clipped by centering the scaled image.
         *
         *      For 2x2 to 20x20, scale the bitmap so that the
         *      specified number of copies will be tiled horizontally
         *      and vertically.
         */
        case BKGND_SCALED:
        {
            // 1x1
            if (pBkgndStore->usScaleFactor < 2)
            {
                ulFlags = SCALEF_EXPAND | SCALEF_CENTER | SCALEF_INCL;
                break;
            }

            // more than 20 suggests corrupted data
            if (pBkgndStore->usScaleFactor > 20)
                return (MRESULT)FALSE;

            // 2x2 to 20x20
            ulFlags = SCALEF_TILED;

            // calc the size of the destination rectl
            szl.cx = pSubCnr->szlCnr.cx / pBkgndStore->usScaleFactor;
            szl.cy = pSubCnr->szlCnr.cy / pBkgndStore->usScaleFactor;

            // calc the number of leftover pixels that have to be
            // dealt with - it could be anywhere from 0 to ScaleFactor-1
            szlAdj.cx = pSubCnr->szlCnr.cx % pBkgndStore->usScaleFactor;
            szlAdj.cy = pSubCnr->szlCnr.cy % pBkgndStore->usScaleFactor;
        }
        break;

        /*
         * BKGND_TILED:
         *      draw the bitmap as many times as it fits in the
         *      container.  If the bitmap is larger than the
         *      window, it will be scaled down proportionally.
         */
        case BKGND_TILED:
        {
            RECTL rcl;
            ...
            // scale the bitmap, but don't center it
            ScaleSrcToDst(&pSubCnr->szlBitmap,
                          &pSubCnr->szlCnr,
                          &rcl,
                          0);

            // save the scaled bitmap's dimensions
            szl.cx = rcl.xRight;
            szl.cy = rcl.yTop;

            // these are used below for tiling BKGND_SCALED bitmaps
            // but not for BKGND_TILED, so set them to zero
            szlAdj.cx = 0;
            szlAdj.cy = 0;
        }
        break;
    ...
    /*
     * Paint routine for BKGND_NORMAL and BKGND_SCALED (1x1)
     */
    if (!(ulFlags & SCALEF_TILED))
    {
        // scale the destination rectl
        ScaleSrcToDst(&pSubCnr->szlBitmap,
                      &pSubCnr->szlCnr,
                      (PRECTL)&aptl[0],
                      ulFlags);

        // display the portion which lies in the update rectl
        BlitClippedBitmap(pob->hps,
                          pSubCnr->hbm,
                          aptl,
                          &pob->rclBackground);
    ...
    /*
     * Paint routine for BKGND_TILED and BKGND_SCALED (2x2 to 20x20)
     *
     * To avoid needlessly painting areas that will be clipped out,
     * calculate the first row (from the top) and the first column
     * that intersect the update rectl - we'll start there.
     *
     * This process is complicated by the possibility that BKGND_SCALED
     * may have had extra pixels that have to be used up by increasing
     * the size of the first so-many rows and/or columns by 1 pixel each,
     * then reverting to the original size for the remainder.  This is
     * not a problem for BKGND_TILED - szlAdj is always zero.
     */

    // calc the first row - if it gets an extra pixel,
    // increase the bitmap height now
    lStart = (pSubCnr->szlCnr.cy - pob->rclBackground.yTop) / szl.cy;
    lAdj = min(lStart, szlAdj.cy);
    if (lStart * szl.cy + lAdj > pSubCnr->szlCnr.cy - pob->rclBackground.yTop)
    {
        lStart--;
        lAdj = min(lStart, szlAdj.cy);
    }
    aptl[1].y = pSubCnr->szlCnr.cy - (lStart * szl.cy + lAdj) - 1;
    szlAdj.cy -= lAdj;
    if (szlAdj.cy)
        szl.cy++;
    aptl[0].y = aptl[1].y + 1 - szl.cy;

    // calc the first column - if it gets an extra pixel,
    // the bitmap width will be increased in the loop below
    lStart = pob->rclBackground.xLeft / szl.cx;
    lAdj = min(lStart, szlAdj.cx);
    if (lStart * szl.cx + lAdj > pob->rclBackground.xLeft)
    {
        lStart--;
        lAdj = min(lStart, szlAdj.cx);
    }
    lStart = lStart * szl.cx + lAdj;
    szlAdj.cx -= lAdj;

    // outer loop: the "rows"
    while (aptl[1].y >= pob->rclBackground.yBottom)
    {
        lAdj = szlAdj.cx;

        // init X values to the first column that has to be painted
        if (lAdj)
            szl.cx++;
        aptl[0].x = lStart;
        aptl[1].x = lStart + szl.cx - 1;

        // inner loop: the "columns"
        while (aptl[0].x < pob->rclBackground.xRight)
        {
            BlitClippedBitmap(pob->hps,
                              pSubCnr->hbm,
                              aptl,
            ...
            // advance rightward - if we've run out of extra pixels,
            // restore the original width for the remaining columns;
            aptl[0].x += szl.cx;
            if (lAdj && !(--lAdj))
                szl.cx--;
            aptl[1].x += szl.cx;
        ...
        // if the last column drawn used an extra pixel,
        // reset the column width to its original size
        if (lAdj)
            szl.cx--;

        // advance downward - if we've run out of extra pixels,
        // restore the original height for the remaining rows
        aptl[1].y -= szl.cy;
        if (szlAdj.cy && !(--szlAdj.cy))
            szl.cy--;
        aptl[0].y -= szl.cy;
    }
 ...
 *
 *@@added V1.0.0 (2002-08-24) [umoeller]
 *@@changed V1.0.9 (2011-10-06) [rwalsh]:  added WM_BUTTON2DOWN intercept
 */
...
MRESULT EXPENTRY fnwpSubclCnr(HWND hwndCnr, ULONG msg, MPARAM mp1, MPARAM mp2)
{
    ...
    switch (msg)
    {
        ...
        // Suppress activation on MB2-down to emulate standard
        // folder behavior.  Putting this here is less of a
        // kludge than it seems:  this is how the WPS does it.
        //@@added V1.0.9 (2011-10-06) [rwalsh]
        case WM_BUTTON2DOWN:
            mrc = (MRESULT)TRUE;
        break;
 ...
 *@@changed V1.0.9 (2011-09-17) [rwalsh]: restructured to improve efficiency
 */
                                  ...
                                  BOOL fFoldersOnly)
{
    ...
    {
        if (!_somIsA(pobj, ((fFoldersOnly) ? _WPFolder : _WPFileSystem)))
        {
            if (!_somIsA(pobj, _WPDisk))
                pobj = NULL;
            else
                pobj = ((_somIsA(pobj, _XFldDisk))
                      ...
                      PCSZ pcszFileMask)     // in: upper-case file mask or NULL
{
    ...
    if (!(pObject = objResolveIfShadow(pObject)))
        // broken shadow - disallow for INSERT_FILESYSTEMS
        return FALSE;

    // filter out non-file systems (shadows pointing to them have been resolved):
    if (!_somIsA(pObject, _WPFileSystem))
        return (_somIsA(pObject, _WPDisk));

    if (!(_wpQueryAttr(pObject) & FILE_HIDDEN))
    {
