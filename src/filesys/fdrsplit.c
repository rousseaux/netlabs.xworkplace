...
MRESULT EXPENTRY fnwpSplitViewFrame(HWND hwndFrame, ULONG msg, MPARAM mp1, MPARAM mp2)
{
    ...
    switch (msg)
    {
        ...
        case WM_INITMENU:
                                                         ...
                                                         QWL_USER))
            {
                ...
                if (ulMenuType)
                {
                    ...
                    if (hwndMenu = _wpDisplayMenu(psvd->psfvBar->somSelf,
                                                  psvd->psfvBar->hwndFrame,
                                                  psvd->psfvBar->hwndCnr,
                                                  NULL,
                                                  ...
                                                  0))
                    {
                        ...
                        fdrManipulatePulldown(psvd->psfvBar,
                                              (ULONG)mp1,
                                              (HWND)mp2);
        ...
        case WM_MENUSELECT:
            ...
            {
                if (psvd->psfvBar)
                    mrc = fdrProcessFolderMsgs(psvd->psfvBar->hwndFrame,
                                               msg,
                                               mp1,
                                               mp2,
                                               psvd->psfvBar,
                                               psvd->psfvBar->pfnwpOriginal);
                else
                    mrc = G_pfnwpSplitFrameOrig(hwndFrame, msg, mp1, mp2);
            }
        break;
                    ...
                    case SN_VKEY:
                        switch ((USHORT)mp2)
                        {
                            ...
                            case VK_BACKTAB:
                            {
                                ...
                                psvd->sv.hwndFocusCnr = hwndFocus;  // V1.0.9 (2012-02-12) [pr]
                                WinSetFocus(HWND_DESKTOP, hwndFocus);
                                // V1.0.5 (2006-04-13) [pr]: Fix status bar update @@fixes 326
                                if (psvd->sv.hwndStatusBar)
                                    WinPostMsg(psvd->sv.hwndStatusBar,
                                               STBM_UPDATESTATUSBAR,
                                               (MPARAM) hwndFocus,
                                               MPNULL);
    ...
    TRY_LOUD(excpt1)
    {
                                               ...
                                               &rc))
        {
                                    ...
                                    pos.lSplitBarPos))
            {
                ...
                psvd->sv.hwndFocusCnr = psvd->sv.hwndTreeCnr;  // V1.0.9 (2012-02-12) [pr]
                WinSetFocus(HWND_DESKTOP,
                            psvd->sv.hwndFocusCnr);
