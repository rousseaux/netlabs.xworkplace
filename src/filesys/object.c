...
HPOINTER CallQueryIconMethod(WPObject *pobj,    // in: object
       ...
       )
    {
        ...
        if (!(pwpQueryIconN
            = (xfTD_wpQueryIconN)wpshResolveFor(pobj,
                                                ...
                                                "wpQueryIconN")))
            cmnLog(__FILE__, __LINE__, __FUNCTION__,
                   "Unable to resolve wpQueryIconN method.");
        else
...
ULONG CallQueryIconDataMethod(WPObject *pobj,    // in: object
                              ULONG ulIndex,     // in: animation index or 0 for regular icon
       ...
       )
    {
        ...
        if (!(pwpQueryIconDataN
            = (xfTD_wpQueryIconDataN)wpshResolveFor(pobj,
                                                    ...
                                                    "wpQueryIconDataN")))
            cmnLog(__FILE__, __LINE__, __FUNCTION__,
                   "Unable to resolve wpQueryIconDataN method.");
        else
...
BOOL CallSetIconDataMethod(WPObject *pobj,    // in: object
                           ULONG ulIndex,     // in: animation index or 0 for regular icon
       ...
       )
    {
        ...
        if (!(pwpSetIconDataN
            = (xfTD_wpSetIconDataN)wpshResolveFor(pobj,
                                                  ...
                                                  "wpSetIconDataN")))
            cmnLog(__FILE__, __LINE__, __FUNCTION__,
                   "Unable to resolve wpSetIconDataN method.");
        else
 ...
 *      this pointer using _wpFreeMem(pobj, pIconInfo).
 *
 *      Otherwise this might return the following errors:
 *
 *      --  ERROR_BAD_FORMAT: icon format not understood.
 *          The WPS docs specify ICON_FILE to be a valid
 *          format, but I have not found that the WPS
 *          actually supports this.
 *
 *      plus those of _wpAllocMem, such as ERROR_NOT_ENOUGH_MEMORY.
 *
 *@@added V0.9.16 (2001-10-15) [umoeller]
 */

APIRET objLoadIconData(WPObject *pobj,             // in: object whose icon to query
                       ULONG ulIndex,              // in: animation index or 0 for regular icon
                       PICONINFO *ppIconInfo)      // out: ICONINFO allocated via _wpAllocMem
{
    ...
    if (cbIconInfo = CallQueryIconDataMethod(pobj, ulIndex, NULL))
    {
        // allocate the memory
        if (pData = (PICONINFO)_wpAllocMem(pobj, cbIconInfo, &arc))
        {
            // ask the object again
            if (CallQueryIconDataMethod(pobj, ulIndex, pData))
            {
                ...
                switch (pData->fFormat)
                {
                    case ICON_RESOURCE:
                        ...
                        if (    (!(arc = DosQueryResourceSize(pData->hmod,
                                                              ...
                                                              pData->resid,
                                                              &pData->cbIconData)))
                             && (!(arc = DosGetResource(pData->hmod,
                                                        RT_POINTER,
                                                        pData->resid,
                                                        &pData->pIconData)))
                            )
                        {
                            pData->fFormat = ICON_DATA;
                        }
                    break;
            ...
            if (!arc)
                *ppIconInfo = pData;
                ...
                _wpFreeMem(pobj, (PBYTE)pData);
        } // end if (pData = _wpAllocMem(pobj, cbIconInfo, &arc))
        // else: wpAllocMem should have set arc properly
        // (does this actually work?)
    } // end if (cbIconInfo = _wpQueryIconData(pobj, NULL))
    else
        arc = ERROR_BAD_FORMAT;     // whatever, this shouldn't fail

    return (arc);
}

/*
 *@@ objCopyIconFromObject:
...
APIRET objCopyIconFromObject(WPObject *somSelf,       // in: target
                             ...
                             ULONG ulIndex)           // in: animation index or 0 for regular icon
{
    ...
    if (pobjSource)
    {
        ...
        if (!(arc = objLoadIconData(pobjSource, ulIndex, &pData)))
        {
            // now set this icon for the target object
            CallSetIconDataMethod(somSelf, ulIndex, pData);
            _wpFreeMem(pobjSource, (PBYTE)pData);
        }
    }
    else
...
{
    ULONG       flIconPageFlags;            // flags for objFormatIconPage so we
               ...
               HPS hps)
{
    ...
    HPOINTER    hptr = CallQueryIconMethod(pData->pcnbp->somSelf,
...
MRESULT EXPENTRY fnwpSubclassedIconStatic(HWND hwndStatic, ULONG msg, MPARAM mp1, MPARAM mp2)
{
    ...
    switch(msg)
    {
        ...
        case DM_DROP:
            if (pData->pobjDragged)
            {
                // dragover was valid above:
                objCopyIconFromObject(pData->pcnbp->somSelf,
                                      pData->pobjDragged,
                                      pData->ulAnimationIndex);
                // and repaint
                RemoveTargetEmphasis(pData, hwndStatic);
                               ...
                               ULONG flFlags)
{
    ...
    if (flFlags & CBI_INIT)
    {
           ...
           )
        {
            ...
            // has NEVERTEMPLATE
            if (    (pData->flIconPageFlags & ICONFL_TEMPLATE)
            ...
            if (flIconPageFlags & ICONFL_ICON)
            {
                ...
                // backup the icon data
                if (pData->ulAnimationIndex)
                    // this page is for the folder animation icon:
                    // compare this icon to the class's animation icon
                    ;       // @@todo
                else
                    // this page is a regular icon:
                    fUsingDefaultIcon = (0 == (_wpQueryStyle(pcnbp->somSelf)
                                                & OBJSTYLE_NOTDEFAULTICON));

                if (!fUsingDefaultIcon)
                    // not default icon: load a copy for "undo"
                    objLoadIconData(pcnbp->somSelf,
                                    pData->ulAnimationIndex,
    ...
    if (flFlags & CBI_ENABLE)
    {
        if (flIconPageFlags & ICONFL_ICON)
        {
            ...
            BOOL fUsingDefault = (pData->pIconDataBackup == NULL);
    ...
    if (flFlags & CBI_DESTROY)
    {
        if (pData)
        {
            ...
            if (pData->pIconDataBackup)
                _wpFreeMem(pcnbp->somSelf,
                           (PBYTE)pData->pIconDataBackup);
...
VOID ResetIcon(POBJICONPAGEDATA pData)
{
    if (pData->flIconPageFlags & ICONFL_ICON)
    {
        ...
        CallSetIconDataMethod(pData->pcnbp->somSelf,
                              pData->ulAnimationIndex,
                              &Data);
    }
}
            ...
            case DID_UNDO:
                ...
                if (flIconPageFlags & ICONFL_ICON)
                {
                    ...
                    if (pData->pIconDataBackup)
                        ...
                        CallSetIconDataMethod(pcnbp->somSelf,
                                              pData->ulAnimationIndex,
                                              pData->pIconDataBackup);
                    else
