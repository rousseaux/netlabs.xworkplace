 ...
 *      of the folder sort items.  If so, it either sets the
 *      folder sort settings accordingly, or sorts the folder
 *      once, depending on which function called it.
 *
 *      This gets called from two locations:
 *
 *      --  from fcmdSelectingFdrMenuItem in the context
 *          of fnwpSubclassedWPFolderWindow to intercept
 *          folder sort menu items so settings can be
 *          updated and the menu left open.
 *
 *      --  from fcmdMenuItemSelected in the context of
 *          XFolder::wpMenuItemSelected to act on requests
 *          to sort the folder.
 *
 *      If this function recognizes the menu ID, it returns
 *      TRUE to prevent further handling even if it takes no
 *      explicit action.
 *
 *      When called by fcmdSelectingFdrMenuItem, pbDismiss
 *      and hwndMenu should be non-null.  If appropriate,
 *      the function will update sort settings and set
 *      *pbDismiss to FALSE to prevent the menu from being
 *      dismissed.
 *
 *      When called by fcmdMenuItemSelected, pbDismiss and
 *      hwndMenu should both be null.  The function uses
 *      pbDismiss as a flag to indicate that the menu has
 *      already been dismissed and that it should sort the
 *      folder based on the selected menuitem.
 *
...
BOOL fdrSortMenuItemSelected(WPFolder *somSelf,
                             HWND hwndFrame,
                             HWND hwndMenu,      // may be NULLHANDLE if pbDismiss is NULL also
                             ULONG ulMenuId,
                             PBOOL pbDismiss)    // out: dismiss menu flag
{
    BOOL            brc = FALSE;
#ifndef __ALWAYSEXTSORT__
    if (cmnQuerySetting(sfExtendedSorting))
#endif
    {
        ...
        if (lSort != -999)
        {
            ...
            // if Shift is pressed, set the default sort criterion;
            // we can assume that the menuitem is being selected rather
            // than has been selected - but it really doesn't matter
            ...
            if (fShiftPressed)
            {
                ...
                // do not dismiss menu V0.9.13 (2001-06-19) [umoeller]
                if (pbDismiss)
                    *pbDismiss = FALSE;
            }
            // use pbDismiss to identify whether the menuitem is being
            // selected (non-null) or has been selected (null);  in this
            // case, it matters or else the folder will be sorted twice
            // changed V1.0.9 (2011-10-21) [rwalsh]
            else if (!pbDismiss)
            {
                // just sort once
 ...
 *@@changed V1.0.9 (2011-09-10) [rwalsh]: rewritten to use object title as a tie-breaker
 */
                                      ...
                                      PVOID pStorage)           // unused
{
    ...
    // changed V1.0.9 (2011-09-10) [rwalsh]
    // Everything below this point was rearranged or rewritten to use an
    // object's title as a fallback when sort values are missing or equal.
    // Without these changes, sorting e.g. the Drives folder by "real name"
    // or "last write date" left the objects in no order at all. No more...

    // Check if each object is descended from the folder's sort
    // class;  the "Class" field was set by fdrQuerySortFunc.
    // If so, calculate the details data field address.
    // The field offset was also set by fdrQuerySortFunc.

    if (f1IsOfSortClass = _somIsA(pobj1, pSortInfo->Class))
        pb1 = (PBYTE*)(  (PBYTE)pmrc1               // object 1's MINIRECORDCORE
                       + sizeof(MINIRECORDCORE)     // skip MINIRECORDCORE --> details data
                       + pSortInfo->ulFieldOffset); // details column field offset

    if (f2IsOfSortClass = _somIsA(pobj2, pSortInfo->Class))
        pb2 = (PBYTE*)(  (PBYTE)pmrc2
                       + sizeof(MINIRECORDCORE)
                       + pSortInfo->ulFieldOffset);

    // if both or neither object is descended from the sort class...
    if (f1IsOfSortClass == f2IsOfSortClass)
    {
        // if both are, compare the details data
        if (pb1)
            lResult = pSortInfo->pfnCompare(pb1, pb2);

        // if the details data was equal OR neither object is descended
        // from the sort class, use the title as a tie-breaker
        if (lResult == CMP_EQUAL)
            lResult = CompareStrings(&(pmrc1->pszIcon), &(pmrc2->pszIcon));
    }
    else
    {
        // one object or the other doesn't support the criterion...

            // semi-kludge:  if we're sorting by Real Name
            // (column 5 for filesystem objects), use the
            // non-filesystem object's title as a substitute
        if (   pSortInfo->lCurrentSort == 5
            && _somDescendedFrom(pSortInfo->Class, _WPFileSystem))
        {
            if (!pb1)
                pb1 = &(pmrc1->pszIcon);
            else
                pb2 = &(pmrc2->pszIcon);
            lResult = CompareStrings(pb1, pb2);
        }
        else
        {
            // otherwise, sort objects that are descended from
            // the sort class higher than those which aren't
            if (f1IsOfSortClass)
                lResult = 1;
            else
                lResult = -1;
 ...
 *@@changed V1.0.9 (2011-10-08) [rwalsh]: added SplitView
 */
                              ...
                              ULONG ulSort)      // sort flag
{
    if (    (ulView == OPEN_CONTENTS)
         || (ulView == OPEN_TREE)
         || (ulView == OPEN_DETAILS)
         || (ulView == *G_pulVarMenuOfs + ID_XFMI_OFS_SPLITVIEW_SHOWING)
       )
    {
        _xwpSortViewOnce(somSelf,
                         hwndView,
