 ...
 *      Returns FALSE if an error occurred, most importantly,
 ...
 *      Returns FALSE if an error occurred, most
...
/*
 *@@ InsertCopyFilename:
 *      adds the "copy filename" submenu to the given menu.
 *      The menuitems vary depending on whether somSelf is one
 *      of a group of selected items.
 *
 *      If the source of the menu is an original file or folder
 *      object, we can see if it's record is selected and if any
 *      other record is selected.
 *
 *      However, if the source was a shadow or disk object, somSelf
 *      refers to the original object and/or the disk object's root
 *      folder.  Since its record isn't inserted into the container,
 *      we can't work with it directly.  Instead, we have to get the
 *      object associated with each selected record, resolve it, then
 *      compare its object pointer to somSelf.
 *
 *@@added V1.0.0 (2002-11-09) [umoeller] @@fixes 219
 *@@changed V1.0.9 (2011-09-17) [rwalsh] rewritten to handle disks and shadows @@fixes 494
 */
                               ...
                               SHORT sPosition)     // in: position or MIT_END
{
    ...
    // If somSelf's record is in the container, this will return TRUE.
    // The record's flRecordAttr member will be updated to reflect its
    // selection status in hwndCnr
    fInserted = (BOOL)WinSendMsg(hwndCnr,
                                 CM_QUERYRECORDINFO,
                                 (MPARAM)&pmrcSelf,
                                 (MPARAM)1);

    // If the record is in the container, this is quick and simple
    if (fInserted)
    {
        // If the record is selected, see if another one is also selected.
        if (pmrcSelf->flRecordAttr & CRA_SELECTED)
        {
            while (!fMultiple)
            {
                pmrcSelected = (PMINIRECORDCORE)WinSendMsg(hwndCnr,
                                                           ...
                                                           (MPARAM)pmrcSelected,
                                                           (MPARAM)CRA_SELECTED);
                if (!pmrcSelected || (ULONG)pmrcSelected == -1)
                    break;
    ...
    else  // Not in the container: resolve and compare object pointers.
    {
        WPObject *pObject;
        ULONG     cSelected = 0;
        BOOL      fSelfSelected = FALSE;

        // Loop until we confirm that somSelf is selected and that at least
        // 2 records are selected (or until we run out of selected records).
        while (!fMultiple)
        {
            pmrcSelected = (PMINIRECORDCORE)WinSendMsg(hwndCnr,
                                                       CM_QUERYRECORDEMPHASIS,
            ...
            if ((pObject = fdrvGetFSFromRecord(pmrcSelected, FALSE)))
            {
                cSelected++;
                if (pObject == somSelf)
                    fSelfSelected = TRUE;
 ...
 *@@changed V1.0.9 (2011-09-11) [rwalsh]: set SplitView as default menuid when appropriate @@fixes 4
 */
                              ...
                              HWND hwndCnr)       // in: cnr hwnd
{
    ...
    TRY_QUIET(excpt1)
    {
        ...
        // changed V1.0.9 (2011-09-11) [rwalsh]: don't add SplitView
        // if the folder can't be opened in Icon view because it's
        // probably a specialized subclass (e.g. OD's Control Center)
        if (    (fOpen = winhQueryMenuItem(hwndMenu,
             ...
             && (WinSendMsg(mi.hwndSubMenu,
                            MM_ISITEMVALID,
                            MPFROM2SHORT(WPMENUID_ICON, FALSE),
                            0))
           )
        {
            ...
            // default view, we have to set the default menuitem because
            // it wasn't present when the WPS created this menu @@fixes 4
            if (_wpQueryDefaultView(somSelf) == ulVarMenuOfs + ID_XFMI_OFS_SPLITVIEW)
                WinSendMsg(mi.hwndSubMenu,
                           MM_SETDEFAULTITEMID,
                           MPFROM2SHORT(ulVarMenuOfs + ID_XFMI_OFS_SPLITVIEW, 0),
                           0);
        }
...
/*      if (cmnQuerySetting(sAddRunItem))
    ...
    CATCH(excpt1)
    {
        ...
        if (pszErrMsg)
        {
            strcpy(pszErrMsg, "An error occurred while XFolder was trying to build "
 ...
 *
 *@@added V1.0.0 (2002-08-31) [umoeller]
 *@@changed V1.0.9 (2011-10-08) [rwalsh]: restrict "folder view" items to standard WPS views
                         ...
                         ULONG ulView)          // in: from wpModifyMenu
{
    ...
    switch (ulMenuType)
    {
        ...
        case MENU_VIEWPULLDOWN:
        {
            ...
            // only add "folder view" items for standard WPS views
            // (i.e. not SplitView or other 3rd-party views)
            // added V1.0.9 (2011-10-08) [rwalsh]
            if (   (ulView == OPEN_TREE)
                || (ulView == OPEN_CONTENTS)
                || (ulView == OPEN_DETAILS)
               )
