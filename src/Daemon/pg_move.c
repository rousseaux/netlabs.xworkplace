                ...
                BOOL bAllowUpdate)
{
    ...
    if (!bAllowUpdate)
        WinEnableWindowUpdate(G_pHookData->hwndXPagerClient, FALSE);
    ...
    if (!bAllowUpdate)
        WinEnableWindowUpdate(G_pHookData->hwndXPagerClient, TRUE);
        ...
        switch (msg)
        {
            ...
            case PGOM_CLICK2LOWER:         // mouse button 2
            {
                ...
                if (msg == PGOM_CLICK2ACTIVATE)
                {
                    ...
                    lDeltaX = ptlMouse.x * pptlMaxDesktops->x / G_szlXPagerClient.cx;
                    lDeltaY = pptlMaxDesktops->y - 1 - ptlMouse.y
                              * pptlMaxDesktops->y / G_szlXPagerClient.cy;
                ...
                if (pXPagerConfig->fClick2Activate)
                {
                    HWND hwndActive;
...
VOID _Optlink fntMoveThread(PTHREADINFO pti)
{
    ...
    // _Pmpf(("Move thread object window is 0x%lX", G_pHookData->hwndXPagerMoveThread));

    if (G_pHookData->hwndXPagerMoveThread
    ...
    WinDestroyWindow(G_pHookData->hwndXPagerMoveThread);
    G_pHookData->hwndXPagerMoveThread = NULLHANDLE;
}
...
VOID pgmmRecoverAllWindows(VOID)
{
    ...
    while ((hwndTemp[usIdx] = WinGetNextWindow(henum)) != NULLHANDLE)
    {
        ...
        if (    ((pswpThis->fl & SWP_HIDE) == 0)
             ...
             && (!WinIsChild(hwndTemp[usIdx], G_pHookData->hwndXPagerFrame))
           )
            usIdx++;
    }
