 ...
 *≥ Kommentar : lÑdt Strings aus Resource Lib - muss vor                   ≥
                         ...
                         PVOID *ppvCnrData)
{
    ...
    do
    {
        ...
        // fÅr Setzen der Controls sorgen
...
/*⁄ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒø
 *≥ Name      : RefreshCnrItem                                             ≥
 *≥ Kommentar : fÅhrt einen Refresh fÅr ein Item durch                     ≥
 *≥             Setzt ausserdem alle Mauszeiger auf den statischen         ≥
 *≥             oder auf den Default-Zeiger zurÅck.                        ≥
 *≥ Autor     : C.Langanke                                                 ≥
 *≥ Datum     : 24.07.1996                                                 ≥
 *≥ énderung  : 24.07.1996                                                 ≥
 ...
 *≥ Aufgaben  : - Refresh durchfÅhren                                      ≥
 *≥ RÅckgabe  : BOOL - Flag, ob erfolgreich                                ≥
 *¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ
 */
    ...
    do
    {
        ...
        for (i = 0; i < NUM_OF_SYSCURSORS; i++)
        {
            ...
            if (ppl->ulPtrCount > 0)
            {
                ...
                // Pointer auf nÑchsten Wert setzen
                // SYS-Pointer setzen
                if (ppl->hptrStatic == 0)
                {
                    // Default setzen
             ...
             i++, pmyrec = (PMYRECORDCORE)pmyrec->rec.preccNextRecord)
        {
            ...
            if (ppl->ulPtrCount > 0)
            {
                ...
                // Werte fÅr Container-Item neu einstellen
        ...
        //   wird hierfÅr kurz der View gewechselt
        ...
        // - Update fÅr die Spaltenbreiten
        if (ulViewStyle & CV_DETAIL)
            WinSendMsg(hwndCnrPointerSet, CM_INVALIDATEDETAILFIELDINFO, 0L, 0L);

        // - ggfs, auf detailview zurÅckschalten
        if (ulViewStyle & CV_DETAIL)
        {
 ...
 *≥ ruft auf  : -                                                          ≥
 *≥ Eingabe   : HWND   - frame window handle                               ≥
 *≥             PULONG - Variable fÅr aktuellen View                       ≥
 *≥ Aufgaben  : - View holen                                               ≥
 *≥ RÅckgabe  : BOOL - Flag, ob erfolgreich                                ≥
 *¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ
 */
 ...
 *≥             Wenn eine Animation geladen ist, wird TRUE zurÅckgegeben   ≥
 *≥ Autor     : C.Langanke                                                 ≥
 *≥ Datum     : 24.07.1996                                                 ≥
 *≥ énderung  : 24.07.1996                                                 ≥
 *≥ aufgerufen: Window Procedure                                           ≥
 ...
 *≥ RÅckgabe  : BOOL - Flag, ob ein Set geladen wurde                      ≥
 *¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ
 */
...
)
{
    ...
    do
    {
        // range fÅr Verarbeitung festlegen
        ...
        // entsprechende Pointerlisten ÅberprÅfen
 ...
 *≥ RÅckgabe  : BOOL - Flag, ob mindestens ein Zeigerbild geladen wurde    ≥
 *¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ
 */
 ...
 *≥ Kommentar : fragt das Animation Flag fÅr einen oder alle Ptr ab        ≥
 *≥             Wenn eine Animation aktiv ist, wird TRUE zurÅckgegeben     ≥
 *≥ Autor     : C.Langanke                                                 ≥
 *≥ Datum     : 24.07.1996                                                 ≥
 *≥ énderung  : 24.07.1996                                                 ≥
 *≥ aufgerufen: Window Procedure                                           ≥
 ...
 *≥ RÅckgabe  : BOOL - Flag, ob eine Animation aktiv ist                   ≥
 *¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ
 */
...
/*⁄ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒø
 *≥ Name      : ToggleAnimate                                              ≥
 *≥ Kommentar : setzt das Animation Flag fÅr einen oder alle Ptr um        ≥
 *≥ Autor     : C.Langanke                                                 ≥
 *≥ Datum     : 24.07.1996                                                 ≥
 *≥ énderung  : 24.07.1996                                                 ≥
 *≥ aufgerufen: Window Procedure                                           ≥
...

{
    ...
    do
    {
        ...
        // ggfs. Flag overrulen
        if (pfEnable)
            fAnimate = !(*pfEnable);    // wird spÑter nochmal negiert !
        ...
        {
            // alle mîglichen Animationen ein- bzw. Ausschalten
 ...
 *≥ Kommentar : lÑdt Resourcen fÅr einen Pointer oder fÅr alle Pointer     ≥
 *≥ Autor     : C.Langanke                                                 ≥
 *≥ Datum     : 20.07.1996                                                 ≥
 *≥ énderung  : 20.07.1996                                                 ≥
 *≥ aufgerufen: Window Procedure                                           ≥
