...
/* Title of the Desktop folder; choose a faily long title,
   ...
   exists */
XFolderMain         = XWorkplace||" Installatie";
/* "Introduction" object (SmartGuide) */
XFolderIntro        = "Introductie";
OnlineReference     = XWorkplace||" Gebruikershandleiding";
/* other objects */
WorkplaceShell      = "Workplace Shell";
XWPSetup            = XWorkplace||" Instellingen";
XWPClassList        = "WPS Class lijst";
OS2Kernel           = OS2||" Kernel";
Screen              = "Beeldscherm";
Media               = "Multimedia";
String              = "Instellingen tekst";
Mouse               = "Muis";
Keyboard            = "Toetsenbord";
Sound               = "Geluid";
XShutdown           = "Uitgebreid afsluiten...";
/* font folder added with V0.9.7 */
FontFolder          = "Letterypes";
...
CreateObject:
    len = length(id);
    if (len == 0) then do
        Say 'Fout bij object "'title'": object ID ontbreekt.';
        exit;
    end

    if (left(id, 1) \= '<') then do
        Say 'Fout bij object "'title'": object ID begint niet met een "<".';
        exit;
    end

    if (right(id, 1) \= '>') then do
        Say 'Fout bij object "'title'": object ID wordt niet afgesloten met een ">".';
        exit;
    end
